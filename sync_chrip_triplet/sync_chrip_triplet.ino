...
/**
 * Execute flash and chirp sequence
 * Creates RGB flash with ascending pitch chirp
 */
void flash_and_chirp(void) {
    // Generate random RGB color
    uint8_t r = get_random_color_component();
    uint8_t g = get_random_color_component();
    uint8_t b = get_random_color_component();
...
int main(void) {
    ...
    bool chirped_at_one_third = false;
    bool chirped_at_two_thirds = false;
    ...
    while (1) {
        ...
        if ((phase >= (PHASE_MAX / 3)) && !chirped_at_one_third) {
            half_chirp();
            chirped_at_one_third = true;
        }

        if ((phase >= (2* PHASE_MAX / 3)) && !chirped_at_two_thirds) {
            half_chirp();
            chirped_at_two_thirds = true;
        }
        ...
        if (phase >= PHASE_MAX) {
            ...
            flash_and_chirp();
            phase = 0;
            chirped_at_one_third = false;
            chirped_at_two_thirds = false;
